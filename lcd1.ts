...
namespace hetaolcd {
     ...
     * @param l light, eg: 20
     */
    //% blockId="light" block="设置背光亮度 %l"
    export function light(l: number) {
    ...
    /**
     * 沙漏效果
     * @param max maxnumber, eg: 1023
     */
    //% blockId="sandglass" block="显示沙漏效果 %str|%q|介于%min|和%max|"
    export function sandglass(str: HetaoLcdposition, q: number, min:number, max:number) {
        q = Math.clamp(min, max, q)
        let addrBuf = Buffer.create(1)
        let dataBuf = Buffer.create(1)
        addrBuf[0] = 129
        if (str == 0) {
    ...
    //% blockId="hetao_lcd_hide_quantity" block="隐藏电量 在 行%y| 列%x|"
    export function hideQuantity(y: number, x: number) {
        let addrBuf = Buffer.create(1)
        let dataBuf = Buffer.create(1)
        addrBuf[0] = y * 16 + x
        dataBuf[0] = 3
    ...
    //% blockId="hetao_lcd_show_quantity" block="显示电量 %str| 在 行%y| 列%x|"
    export function showQuantity(str: HetaoLcdposition, y: number, x: number) {
        let addrBuf = Buffer.create(1)
        let dataBuf = Buffer.create(1)
        if(str == 0){
            addrBuf[0] = y * 16 + x
